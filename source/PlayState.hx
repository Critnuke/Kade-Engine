...
class PlayState extends MusicBeatState
{
	...
	private var shakin:Bool = false;
	private var angry:Bool = false;
	private var mad:Bool = false;
	private var mad1:Bool = false;
	private var mad2:Bool = false;
	...
	private var bg:FlxSprite = new FlxSprite(-650, -440); // I'm so sorry
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'ticket-to-tartarus':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ticket-to-tartarus/dialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'RealityBendM':
				{
					defaultCamZoom = 0.7;
					curStage = 'RealityBendM';
					var bg:FlxSprite = new FlxSprite(-650, -440);
					bg.loadGraphic(Paths.image('background/ashmbgc'));
					add(bg);
					bg.active = false;
					bg.updateHitbox();
					bg.antialiasing = true;
	...
	override public function update(elapsed:Float)
	{
		if (shakin)
		{
			FlxG.camera.shake(0.02, 0.02); //0.05
		}
		#if !debug
		perfectMode = false;
		#end
		...
		if (curSong.toLowerCase() == 'stylish sacrifice')
			{
				switch (angry)
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						daNote.kill();
						//This makes Ash's notes affect the HP bar. You can, why can't she? 
						if (curSong.toLowerCase() == 'ticket to tartarus')
						{
							if (healthBar.percent > 1)
								{
									health -= 0.012;
								}
							if (healthBar.percent > 20 && mad2)
								{
									health -= 0.02;
								}
						}
						if (curSong.toLowerCase() == 'stylish sacrifice' && angry)
						{
							if (healthBar.percent > 50 && !mad && !mad1)
								{
									health -= 0.009;
								}
							if (healthBar.percent > 40 && mad)
								{
									health -= 0.012;
								}
							if (healthBar.percent > 21 && mad1)
								{
									health -= 0.04;
								}
						}
	...
	*/
	...
	var herewego:FlxSprite = new FlxSprite(-650, -440);

	override function beatHit()
	{
		super.beatHit();
		
		//THIS ABSOLUTELY REFUSES TO WORK! WHY?!?!?!?!? 00:15
		//Does it not work on Kade Engine?? 01:38
		//I'm losing my mind. 01:38
		//WHY DOES THIS ONLY WORK WITH CURBEAT?? AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA 02:23
		if (SONG.song.toLowerCase() == 'ticket to tartarus')
		{
			switch (curBeat)
			{
				case 15:
					//I know how bad this probably is.
					//Someone please teach me how to do this better. This is the only way I know to make the layers work.
					shakin = true;
					...
					add(dad);
					add(boyfriend);
					//herewego.loadGraphic(Paths.image('background/ashbgc'));
					//add(herewego);
					//herewego.active = false;
					//herewego.updateHitbox();
					//herewego.antialiasing = true;
					//herewego.scrollFactor.set(0.9, 0.9);
					FlxG.log.add('IT WORKED!');
				case 17:
					shakin = false;
				case 312:
					mad2 = true;
		...
		if (SONG.song.toLowerCase() == 'stylish sacrifice')
		{
			switch (curBeat)
			{
				case 352:
